...
@annotate('INT,INT,INT,INT,Double->Double')
class KDENSITY(object):

    def __init__(self):
        self.alpha_dict = {0: 111.1949, 1: 111.178, 2: 111.1272, 3: 111.0425, 4: 110.9241, 5: 110.7718, 6: 110.5858, 7: 110.3661,
        ...
        import numpy as np
        self.logStandardDeviationPlusHalfLog2Pi = np.log(20.0) + 0.5 * np.log(2 * np.pi)

    def evaluate(self, lon1,lat1,lon2,lat2,bandwidth):
        import numpy as np
        distance = np.sqrt((((lon2 - lon1)/120000) ** 2) + (((lat2 - lat1)/120000) ** 2)) * self.alpha_dict[abs(int(lat1/120000-90))]

        if distance < bandwidth:
            #logStandardDeviationPlusHalfLog2Pi = np.log(bandwidth) + 0.5 * np.log(2 * np.pi)
            t = (distance/ bandwidth)
            gaussian_value = np.exp( (-1/2)  * t * t - self.logStandardDeviationPlusHalfLog2Pi)
            return float(gaussian_value)
        return 1e-6
